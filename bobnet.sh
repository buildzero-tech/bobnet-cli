...
cmd_trust() {
    ...
    case "$subcmd" in
        ...
        show) trust_show "$@" ;;
        set) trust_set "$@" ;;
        archive) trust_archive "$@" ;;
        restore) trust_restore "$@" ;;
        delete) trust_delete "$@" ;;
        cleanup) trust_cleanup "$@" ;;
        export) trust_export "$@" ;;
        import) trust_import "$@" ;;
        -h|--help|help)
            cat <<'EOF'
...
COMMANDS:
  ...
  archive <email>     Archive contact
  restore <email>     Restore archived/deleted contact
  delete <email>      Delete contact
  cleanup             Cleanup stale contacts
...
EOF
    ...
    if [[ -n "$archive_candidates" ]]; then
        ...
        local archive_count=$(echo "$archive_candidates" | wc -l | tr -d ' ')
        
        if [[ "$dry_run" == "false" ]]; then
            if [[ "$auto_yes" == "false" ]]; then
                read -p "Archive $archive_count contact(s)? [y/N] " -n 1 -r
                echo
                [[ ! $REPLY =~ ^[Yy]$ ]] && echo "Skipped archive." || {
